...
function RevivePlayer(playerPed)
    ...
    if IsPedDeadOrDying(closestPlayerPed, 1) then
        ...
        TriggerEvent("vorp:TipBottom", T.reviveInProg, 2000)
        Wait(2000)
        ClearPedTasksImmediately(PlayerPedId())
        TriggerServerEvent('poke_medic:revivePlayer', GetPlayerServerId(playerPed))
        WarMenu.CloseMenu()
    else
        TriggerEvent("vorp:TipBottom", T.playerNotUnconscious, 3000)
    end
end

function HealPlayer(playerPed)
    TaskStartScenarioInPlace(PlayerPedId(), GetHashKey('WORLD_HUMAN_CROUCH_INSPECT'), 10000, true, false, false, false)
    TriggerEvent("vorp:TipBottom", T.healInProg, 10000)
    Wait(10000)
    ClearPedTasksImmediately(PlayerPedId())
...
function SpawnHorse()
    ...
    local myHorse = Citizen.InvokeNative(0xD49F9B0955C367DE, model, pos.x + 5, pos.y + 5, pos.z, 0.0, true, true, true,
        true)
